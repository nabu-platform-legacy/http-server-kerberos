 ...
 */
public class KerberosAuthenticationListener implements EventHandler<HTTPRequest, HTTPResponse> {
	...
	public KerberosAuthenticationListener(String jaasConfigurationName, String kerberosRealm, String username, String password, RealmHandler realmHandler) {
		this.jaasConfigurationName = jaasConfigurationName;
		this.kerberosRealm = kerberosRealm;
		this.realmHandler = realmHandler;
		initialize(jaasConfigurationName, username, password);
	}
	
	public KerberosAuthenticationListener(String jaasConfigurationName, String kerberosRealm, String username, String password) {
		this(jaasConfigurationName, kerberosRealm, username, password, null);
	}
	
	private void initialize(String jaasConfigurationName, String username, String password) {
		try {
			// if no kerberos realm is given, assume the default realm
			loginContext = new LoginContext(jaasConfigurationName, newPasswordHandler(username + (kerberosRealm == null ? "" : "@" + kerberosRealm), password));
			loginContext.login();
		}
				...
				if (header != null && header.getValue().substring(0, 9).equalsIgnoreCase("Negotiate")) {
					...
					if (!token.isEmpty()) {
						...
						if (!context.isEstablished()) {
							...
							Token kerberosToken = authenticator != null
								? authenticator.authenticate(jaasConfigurationName, realm, new KerberosIdentity(srcName.toString(), srcName.isAnonymous(), lifetime)) 
								: new KerberosToken(
										realm, 
										lifetime, 
							...
							request.getContent().setHeader(new SimpleAuthenticationHeader(kerberosToken));
							context.dispose();
							return null;
						}
					}
	...
	public void setAuthenticator(KerberosAuthenticator authenticator) {
